...
# Embedding Layer: Chuyển đổi các từ thành các vector nhiều chiều để nắm bắt thông tin ngữ nghĩa và cú pháp.
...
print('==> Kmean Summarizing successfully')
...
with open(f"E:/DATN_Thacsi/data/result_train_test_{current_date}_sentoken.txt", "w", encoding="utf-8") as output:
    for item in result:
        output.write("%s\n" % item)
...
lines_train_test = []
with open(f'E:/DATN_Thacsi/data/result_train_test_{current_date}_sentoken.txt', encoding="utf-8") as file:
    for line in file:
...
for i in data_test.index:
    ...
    rouge_1_test.append(list(scores['rouge1'][0:3]))
